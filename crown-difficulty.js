...
function terminalScore(chess, ply) {
  if (chess.isCheckmate()) return chess.turn() === 'b' ? -100000 + ply : 100000 - ply;
  ...
  for (const rank of ['1', '2', '3', '4', '5', '6', '7', '8']) {
    for (const file of ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h']) {
      ...
      if (['d4', 'e4', 'd5', 'e5'].includes(`${file}${rank}`)) {
        score += piece.color === 'b' ? 16 : -16;
      }
      if (piece.type !== 'p' && piece.type !== 'k' && ['a1', 'h1', 'a8', 'h8'].includes(`${file}${rank}`)) {
        score += piece.color === 'b' ? -8 : 8;
      }
    }
  }

  const mobility = chess.moves().length;
  score += chess.turn() === 'b' ? mobility * 1.4 : -mobility * 1.4;
  return score;
}

function moveScore(chess, move) {
  let score = 0;
...
function orderedMoves(chess) {
  return chess.moves({ verbose: true }).sort((a, b) => moveScore(chess, b) - moveScore(chess, a));
}
...
function chooseHeuristic(chess, moves) {
  const scored = moves.map((move) => ({ move, score: moveScore(chess, move) + Math.random() * 55 }));
  scored.sort((a, b) => b.score - a.score);
  const pool = scored.slice(0, Math.min(5, scored.length));
  return chooseRandom(pool).move;
}

function chooseMinimax(chess, moves, depth) {
  ...
  for (const move of moves) {
    ...
    chess.undo();
    const adjusted = score + moveScore(chess, move) * 0.03;
...
export function chooseCrownMove(chess, difficulty = 'crown') {
  ...
  if (level.mode === 'random') return chooseRandom(moves);
  if (level.mode === 'heuristic') return chooseHeuristic(chess, moves);
  return chooseMinimax(chess, moves, level.depth);
}
...
export function replayFromSan(sans = []) {
